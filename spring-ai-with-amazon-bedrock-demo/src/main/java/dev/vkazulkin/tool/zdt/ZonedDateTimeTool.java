...
public class ZonedDateTimeTool {
	...
	private static final Logger logger = LoggerFactory.getLogger(ZonedDateTimeTool.class);
	
    @Tool(name="Current_Date_Time_Tool", description = "Provide the current date")
    String getZonedDateTime() {
    	logger.info("Current_Date_Time_Tool returned: "+LocalDateTime.now().toString());
        return LocalDateTime.now().toString();
    }
    
    @Tool(name="Current_Time_Zone", description = "Provide the current time zone")
    String getTimeZone() {
    	logger.info("Current_Time_Zone returned: "+TimeZone.getDefault().toString());
